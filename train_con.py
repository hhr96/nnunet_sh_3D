    ...
    with torch.no_grad():
        ...
        for input, target, input_gray, _ in val_loader:
            ...
            sample_image = torch.cat(
                (torch.unsqueeze(input_gray[0, :, 2, :, :], 0), torch.unsqueeze(target[0, :, :, :], 0)), 0)
            ...
            if config['deep_supervision']:
                ...
                for i in range(1, len(val_img_ids)):
                    sample_image = torch.cat((sample_image, torch.unsqueeze(input_gray[i, :, 2, :, :], 0),
...
def main():
    ...
    # Data loading code
    train_img_ids = glob(os.path.join('inputs', config['dataset'], 'train', 'masks','0', '*' + config['img_ext']))
    train_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids] # 将图像的名字从文件里取出
    train_patient_ids = [train_img_ids[x].split('_', 2)[1] for x in range(len(train_img_ids))]
    val_img_ids = glob(os.path.join('inputs', config['dataset'], 'val', 'masks','0', '*' + config['img_ext']))
    val_img_ids = [os.path.splitext(os.path.basename(q))[0] for q in val_img_ids]  # 将图像的名字从文件里取出
    val_patient_ids = [val_img_ids[x].split('_', 2)[1] for x in range(len(val_img_ids))]
