...
def parse_args():
    ...
    parser.add_argument('--nesterov', default=False, type=str2bool, #计算未来超前点的斜率，更快的sgd，过minimum会跳回来
                        help='nesterov')
    parser.add_argument('--weight_decay', default=1e-4, type=float, #当越接近最低值得时候，weight decay可以让更新的值变小， 更不容易出minimum https://towardsdatascience.com/why-adamw-matters-736223f31b5d, L2cost
                        help='weight decay')
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target, _, _ in val_loader:
            ...
            avg_meters['iou'].update(iou, input.size(0)) #更新loss跟iou的值
...
def validate_save(config, val_loader, model, criterion, epoch, val_img_ids):
    avg_meters = {'loss': AverageMeter(),
                  'iou': AverageMeter()}
    ...
    with torch.no_grad():
        ...
        for input, target, input_gray, _ in val_loader:
            ...
            sample_image = torch.cat((torch.unsqueeze(input_gray[0, :, :, :], 0), torch.unsqueeze(target[0, :, :, :], 0)), 0)
            ...
            if config['deep_supervision']:
                ...
                for i in range(1, len(val_img_ids)):
                    sample_image = torch.cat((sample_image, torch.unsqueeze(input_gray[i, :, :, :], 0),
                                              torch.unsqueeze(target[i, :, :, :], 0), torch.unsqueeze(output[i, :, :, :], 0)), 0)
                loss = criterion(output, target)
                iou = iou_score(output, target)
...
def main():
    ...
    if config['optimizer'] == 'Adam':
        ...
        optimizer = optim.SGD(params, lr=config['lr'], momentum=config['momentum'],
                              nesterov=config['nesterov'], weight_decay=config['weight_decay'])#设定是什么optimizer 要么是adam要么是sgd
    else:
    ...
    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids] #将图像的名字从文件里取出

    train_img_ids, val_img_ids = train_test_split(img_ids, test_size=config['val_set_ratio'], random_state=41)#选取validation image
    ...
    for epoch in range(config['epochs']):
        ...
        if np.mod(epoch, config['sample_freq']) == 0:
            val_log = validate_save(config, val_loader, model, criterion, epoch, val_img_ids) #也就只输出两个值loss，iou
        else:
